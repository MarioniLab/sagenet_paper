...
sce_ref  = sce[, sce$embryo == ref_tag]
cell_idx = which(sce$embryo %in% c('scRNAseq', 'embryo1_5', 'embryo2_5', 'embryo3_5'))
sce %<>% .[, cell_idx]
...
if(overwrite | 
  ...
  # True physical space
  types = intersect(sce_ref$cell_type, sce$cell_type)
  cells_ref = meta_ref[cell_type %in% types]$cell_id
  cells_query = meta_dt[cell_type %in% types & embryo == 'scRNAseq']$cell_id
  ccc_list = list()
  g_out    = map_2d[['true']][cells_ref, ] %>%
    get_delaunay(plot=FALSE) %>%
    .$graph
  ccc_list[['true']] = cellCellContact(
      sce = sce_ref[, cells_ref],
      group = 'cell_type',
      graph = g_out,
      nperm = 500,
      plot = FALSE,
...
if(overwrite | 
  ...
  preds_list[['sagenet']] = map_2d[['sagenet']][cells, ]
  preds_list[['exprs']] = map_2d[['exprs']][cells, ]
  preds_list[['sagenet_markers']] = map_2d[['sagenet_markers']][cells, ]
  preds_list[['tangram_cells']] = map_2d[['tangram_cells']][cells, ]
  preds_list[['novosparc']] = map_2d[['novosparc']][cells, ]
  preds_list[['true']] = meta_ref[, .(x,y)]
  # lisi_list   = list()
  lisi_vals_true = compute_lisi(preds_list[['true']], meta_ref, c("cell_type")) %>%
      as.data.table %>% 
      .[, `:=`(cell_id = ..meta_ref$cell_id, method = 'true')]
  lisi_vals = setdiff(names(preds_list), 'true') %>%
    map(~compute_lisi(preds_list[[.x]], meta_dt[cells], c("cell_type")) %>%
      as.data.table %>% 
      .[, `:=`(cell_id = ..cells, method = .x)]) %>%
      purrr::reduce(rbind)
...
```{r C, fig.height=4, fig.width=5, message=FALSE, cache=FALSE}
# fig_list[['C2']] = ccc_dt %>% 
#   ggplot +
