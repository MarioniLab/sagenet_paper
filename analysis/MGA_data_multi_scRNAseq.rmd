...
# wflow_build(files='analysis/MGA_data_multi_scRNAseq.rmd', view=F, verbose=T, delete_cache=T)
```
...
sce_f = 'data_tidy/seqfish_mouse_embryo/embryo1_2.h5ad'
methods     = list.files(out_dir)
names(methods) = methods
preds_f   = methods %>%
  map(~file.path(out_dir, .x, paste0(paste('preds', ref_tag, 'query_data', sep='_'), '.h5ad')))
preds_f[['sagenet']] = 'output/seqfish_mouse_embryo/sagenet/preds_embryo3_2_query_data.h5ad'
imp_f = file.path('int_data/seqfish_mouse_embryo', 'gene_dt_embryo1_2.txt')
col_dt_f = file.path('int_data/seqfish_mouse_embryo', 'col_dt_embryo1_2.txt')
...
sce_query = sce[, sce$embryo == query_tag]
...
if(overwrite | 
  ...
  rownames(d_mtx[['sagenet']]) = colnames(d_mtx[['sagenet']]) = colnames(sce_query)
  map_2d[['sagenet']] = Rtsne(d_mtx[['sagenet']], is_distance=T, perplexity=50)$Y
  rownames(map_2d[['sagenet']]) = colnames(sce_query)
  meta_query[, `:=`(sagenet_1 = ..map_2d[['sagenet']][cell_id, 1], 
  ...
  d[['sagenet_markers']] = dist(t(assays(sce_query[markers,])[[1]])) %>% as.dist
  d_mtx[['sagenet_markers']] = d[['sagenet_markers']] %>% as.matrix 
  rownames(d_mtx[['sagenet_markers']]) = colnames(d_mtx[['sagenet_markers']]) = colnames(sce_query)
  ...
  rownames(map_2d[['sagenet_markers']]) = colnames(sce_query)
  meta_query[, `:=`(markers_tsne_1 = ..map_2d[['sagenet_markers']][cell_id, 1], 
  ...
  rownames(d_mtx[['tangram_cells']]) = colnames(d_mtx[['tangram_cells']]) = meta_query$cell_id
  rownames(map_2d[['tangram_cells']]) = colnames(sce_query)
  meta_query[, `:=`(tangram_1 = ..map_2d[['tangram_cells']][cell_id, 1], 
                tangram_2 = ..map_2d[['tangram_cells']][cell_id, 2])]

  d[['novosparc']] = dist(preds_list[['novosparc']])
  d_mtx[['novosparc']] = d[['novosparc']] %>% as.matrix
  rownames(d_mtx[['novosparc']]) = colnames(d_mtx[['novosparc']]) = meta_query$cell_id
  rownames(map_2d[['novosparc']]) = colnames(sce_query)
  meta_query[, `:=`(novosparc_1 = ..map_2d[['novosparc']][cell_id, 1], 
...
ccc_dt = names(ccc_dist) %>%
  map(~data.table(method=.x, embryo=y, corr=ccc_dist[[.x]])) %>% purrr::reduce(rbind) 
...
meta_dt %>% setkey(cell_id)
meta_ref %>% setkey(cell_id)
preds_list = list()
if(overwrite | 
  !file.exists(file.path(obj_dir, 'lisi_vals.Rds'))
){
  preds_list[['sagenet']] = map_2d[['sagenet']][cells_query, ]
  preds_list[['exprs']] = map_2d[['exprs']][cells_query, ]
  preds_list[['sagenet_markers']] = map_2d[['sagenet_markers']][cells_query, ]
  preds_list[['tangram_cells']] = map_2d[['tangram_cells']][cells_query, ]
  preds_list[['novosparc']] = map_2d[['novosparc']][cells_query, ]
  ...
  lisi_vals = setdiff(names(preds_list), 'true') %>%
    map(~compute_lisi(preds_list[[.x]], meta_dt[cells_query], c("cell_type")) %>%
      as.data.table %>% 
...
meta_dt[, ent :=  5 - (ent_embryo3_2_leiden_1 + ent_embryo3_2_leiden_0.1 + ent_embryo3_2_leiden_0.01 + ent_embryo3_2_leiden_0.5 + ent_embryo3_2_leiden_0.05) ]
...
## B: UMAP plosts of expression space and the reconstructed space
```{r BS, fig.height=10, fig.width=15, message=FALSE, cache=FALSE, eval=TRUE}
...
fig_list[['BS']] = exprs_p + markers_p + sagenet_p + novosparc_p + tangram_p  + lgened +
  plot_layout(ncol = 3)
fig_list[['BS']]
