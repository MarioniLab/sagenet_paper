...
# wflow_build(files='analysis/DHH_data_ST.rmd', view=F, verbose=T, delete_cache=T)
```
...
ccc_plot = cellCellContactHeatmapTriangle(pmat, class__cols, title='')
...
```{r B, fig.height=4, fig.width=12, message=FALSE, cache=FALSE, eval=TRUE}
# meta_dt %<>% .[, .SD[sample(.N)]]
meta_dt %<>% 
...
fig_list[['B1']] = sagenet_p  + sagenet_grid_p + sagenet_batch_p + plot_layout(ncol=3, guides='collect')
fig_list[['B1']]
...
fig_list[['B2']] = exprs_p  + exprs_grid_p + exprs_batch_p + plot_layout(ncol=3, guides='collect')
fig_list[['B2']]
```
...
```{r XN2, fig.height=24, fig.width=24, message=FALSE, cache=FALSE, eval=TRUE}

# p_list = list()
# for(m in intersect(markers, rownames(s_sce))){

#   p_list[[m]] = plot_2d_cont( 
#     dim_df = -map_2d[['sagenet']][meta_dt$cell_id,],
...
# }
# p_list %<>% purrr::reduce(`+`) + plot_layout(ncol=6)
# p_list
...
## XA: GGM
```{r XA, fig.height=10, fig.width=10, message=FALSE, cache=FALSE}
...
graph_col_comm(graph=g_obj, lay=lay, grp=grp, title='GIN on ST dataset', labels=rownames(adj))
